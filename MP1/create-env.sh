...
DB_SECURITY_GROUP_NAME=$(aws rds describe-db-instances --db-instance-identifier $DB_ID --query "DBInstances[*].DBSecurityGroups[*].DBSecurityGroupName[0]" --output=text)
aws rds authorize-db-security-group-ingress --db-security-group-name $DB_SECURITY_GROUP_NAME --ec2-security-group-id $SECURITY_GROUP
